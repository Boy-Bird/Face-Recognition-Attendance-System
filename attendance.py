...
class Attendance:
    def __init__(self,root):
        ...
        title_lbl = Label(self.root, text='ATTENDANCE MANAGEMENT SYSTEM', font=('times new roman', 35, 'bold'),
                          bg='white', fg='black')
        title_lbl.place(x=0, y=180, width=1530, height=40)

        main_frame = Frame(self.root, bd=2)
        main_frame.place(x=0, y=230, width=1500, height=600)
        ...
        left_inside_frame.place(x=0, y=135, width=720, height=390)

        # ======== Labels and Entry =======attendanceId# attendance id
        ...
        atten_name = ttk.Entry(left_inside_frame, width=21,textvariable=self.var_atten_name,
        ...
        atten_time = ttk.Entry(left_inside_frame, width=21,textvariable = self.var_atten_time,
                               font=("times new roman", 11))
        ...
        atten_date = ttk.Entry(left_inside_frame, width=21,textvariable=self.var_atten_date,
                               font=("times new roman", 11))
        ...
        save_btn = Button(btn_frame, text='Import csv',command = self.importCsv, width=19, font=("times new roman", 13),
                          bg='black', fg='white')
        save_btn.grid(row=0, column=0)

        update_btn = Button(btn_frame, text='Export csv',command=self.exportCsv,  width=19, font=("times new roman", 13),
                            bg='black', fg='white')
        update_btn.grid(row=0, column=1)

        delete_btn = Button(btn_frame, text='Update',  width=19, font=("times new roman", 13),
                            bg='black', fg='white')
        delete_btn.grid(row=0, column=2)

        reset_btn = Button(btn_frame, text='Reset',command=self.reset_data,  width=19, font=("times new roman", 13),
                           bg='black', fg='white')
    ...
    def exportCsv(self):
        try:
            if len(mydata) < 1:
                ...
                return False
            file = filedialog.asksaveasfilename(initialdir=os.getcwd(), title="Open CSV",
            ...
            with open(file, "w", newline="") as myfile:
                ...
                for i in mydata:
                    export.writerow(i)
                messagebox.showinfo("Success", "Data Exported to "+os.path.basename(file))
    ...
    def reset_data(self):
        ...
        # img_right = Image.open(r"C:\Users\yashp\OneDrive\Documents\FaceRecog\images\graybg.jpg")
